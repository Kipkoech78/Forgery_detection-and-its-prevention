...
<tbody>
<?php
$ordered=mysqli_query($conn,"SELECT * FROM purchase WHERE status='decline'");
...
echo'<tr><td>'.$count.'</td><td>'.$fullname.'</td><td>'.$prodtype.'</td><td>'.$date.'</td><td>'.$status.'</td><td>'.$quantity.'</td><td>'.$total.'</td><td>'.$pay.'</td><td>'.$period.'</td><td><a href="decline.php?id='.$actno.'" class="btn btn-danger"><span class="glyphicon glyphicon-remove"></span> Decline</a></td></tr>';
$count++;
}
?>
</tbody>
