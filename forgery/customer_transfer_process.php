...
if (isset($_SESSION["login_id"])) 
{
   ...
   //select the last transaction id in the sender's passbook
    ...
    if($s_amount<=500)
    {
        echo '<script>alert("Your account balance is less than Ksh. 500.\n\nYou must maintain a minimum balance of Ksh. 500 in order to proceed with the transfer.");';
        echo 'window.location= "customer_transfer.php";</script>';
    }
    elseif($t_amount<100){
         echo '<script>alert("You cannot transfer less than Ksh. 100");';
        echo 'window.location= "customer_transfer.php";</script>';
    }
    elseif($s_total<500)
    {
        echo '<script>alert("You do not have enough balance in your account to proceed this transfer.\n\nYou must maintain a minimum of Ksh. 500 in your account.");';
        echo 'window.location= "customer_transfer.php";</script>';
    }
